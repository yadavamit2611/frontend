...
export default function AdminEdit({name,designation,email,gender,location,pass,_id}) {
  const [open, setOpen] = useState(false);
  const [user_id, setUser_Id] = useState(_id);
  const [values, setValues] = useState({name,designation,email,gender,location,pass});
  ...
  const handleSubmit = async (e) => {
        ...
        try {
          ...
          const add_url = 'http://localhost:5000/admin/updateAdmin';
  ...
  return (
    <>
  <Button onClick={handleOpen} >Edit Admin</Button>
        <Dialog
        size="xs"
        open={open}
        handler={handleOpen}
        ...
        <Card className="mx-auto w-full max-w-[24rem]">
          ...
          <form onSubmit={handleSubmit}>
            <CardBody className="flex flex-col gap-4">
              ...
              <Input label="Password" type="password" name="pass" value={values.pass} onChange={handleInputChange} size="lg" />
